...
% You should run this when compiling a file for the first time, but it is recommended to comment
% it out once nag has been satisfied since performing the check adds compilation time.
...
% Use a smaller point for figure captions
\usepackage[font=small,labelfont=bf]{caption}

% The hidelinks option to hyperref removes surrounding boxes in ref links. The breaklinks option
% allows line breaking for links.
\usepackage[hidelinks,breaklinks]{hyperref}
% Replaces \ref and \eqref with ``clever'' \cref command.
% should be loaded after other packages such as hyperref
% \usepackage{cleveref}
% The breakurl package option is specific to urls.
...
% \include{qcircuit_custom_commands}
...
% Usually you will want this as well when using math
\usepackage{amssymb}

% Some useful fraction macros
\newcommand\frachalf{%
  \frac{1}{2}}
...
% in your (compiled) wordcounts (see below). To ignore them at the .tex level
% you must use a %TC directive (see below).
\newcommand{\ignore}[1]{}
...
% Useful macro that allows you to save the value of a variable. E.g.:
%   \setvalue{\myvar}{5}
%   \setvalue{\savedpage}{\thepage}
% Then you can refer to the variable simply by name from that point forward:
%   \savedpage
%   \myvar
...
% Set these when using fancy headers (enable the fancyhdr package above)
...
% These are used to pass options to texcount. Here we are telling it not to ignore citations in word counts, and to ignore comments:
...
%TC:macro \comment [ignore]
%TC:macro \mike [ignore]
%TC:macro \stephan [ignore]
